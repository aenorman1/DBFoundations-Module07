...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AleshaNorman')
	 Begin 
	  Alter Database [Assignment07DB_AleshaNorman] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AleshaNorman;
	 End
	Create Database Assignment07DB_AleshaNorman;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AleshaNorman;
...
Create or Alter View vProductInventories
 As
	Select Top 1000000
		...
		--InventoryDate = DateName(mm, InventoryDate) + ', ' + DateName(yyyy, InventoryDate), -- alternate way
		[Count] as InventoryCount
	From vProducts
	 Join vInventories
		 on vProducts.ProductID = vInventories.ProductID
Order by ProductName, Cast(Format(InventoryDate, 'MMMM, yyyy') As Date) -- trying out another format option
Go
...
Select Top 1000000
	...
	CountVsPreviousCountKPI = Case
		...
		When InventoryCount = PreviousMonthCount Then 0 -- order matters here, cannot have the = first
		When InventoryCount < PreviousMonthCount Then -1
	End
  ...
  As
	Return
	    ...
	    From vProductInventoriesWithPreviousMonthCountsWithKPIs
		 Where vProductInventoriesWithPreviousMonthCountsWithKPIs.CountVsPreviousCountKPI = @KPI
Order By ProductName, Cast(InventoryDate as Date)
Go
